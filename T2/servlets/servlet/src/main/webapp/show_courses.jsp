<%@ page import="javax.persistence.EntityManagerFactory" %>
<%@ page import="javax.persistence.Persistence" %>
<%@ page import="javax.persistence.EntityManager" %>
<%@ page import="ejb.CourseEntity" %>
<%@ page import="javax.persistence.TypedQuery" %>
<%@ page import="java.util.List" %>
<!DOCTYPE html>
<html>
<head>
    <title>FetchCourseList</title>
</head>
<body>
<h2>Lista cursurilor</h2>
<table>
    <thead>
    <tr>
        <th>ID</th>
        <th>Nume</th>
        <th>Universitate</th>
        <th>Numar studenti</th>
        <th></th>
        <th></th>
    </tr>
    </thead>
    <tbody>
    <%
        EntityManagerFactory factory =
                Persistence.createEntityManagerFactory("bazaDeDateSQLite");

        EntityManager em = factory.createEntityManager();

        TypedQuery<CourseEntity> query = em.createQuery("SELECT Course FROM CourseEntity Course", CourseEntity.class);

        List<CourseEntity> results = query.getResultList();
    %>


    <% for (int i = 0; i < results.size(); ++i){ %>
    <% CourseEntity result = results.get(i); %>
    <tr>
        <td>
            <form id="myform<%= i %>" action="./update-course" method="post">
                <input type="text" disabled value="<%= result.getId() %>"/>
                <input type="hidden" name="id" value="<%= result.getId() %>"/>
            </form>
        </td>
        <td><input form="myform<%= i %>" type="text" name="nume" value="<%= result.getNume() %>"/></td>
        <td><input form="myform<%= i %>" type="text" name="universitate" value="<%= result.getUniversitate() %>"/></td>
        <td><input form="myform<%= i %>" type="number" name="nrStudenti" value="<%= result.getNumarStudenti() %>"/></td>
        <td>
            <form action="./delete-course" method="post">
                <input type="hidden" name="id" value="<%= result.getId() %>"/>
                <input type="submit" value="Delete"/>
            </form>
        </td>
        <td>
            <input form="myform<%= i %>" type="submit" value="Update"/>
        </td>
    </tr>
    </tbody>
    <% } %>
    <%
        em.close();
        factory.close();
    %>
</table>
</body>
</html>
